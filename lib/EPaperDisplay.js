...
class EPaperDisplay extends EventEmitter {
    ...
    async queueDisplayBuffer(displayBuffer) {
        try {
            const newContent = {
                displayBuffer: displayBuffer,
                timestamp: Date.now()
            ...
            if (this.isDrawing) {
                this.queuedContent = newContent;
                Logger.log(this.deviceId, 'info', 'Display buffer queued while display is busy');
            } else {
                this.contentToDisplay = newContent;
                await this.displayImage(displayBuffer);
            }
            ...
            Logger.log(this.deviceId, 'error', `Error displaying buffer: ${error}`);
            throw error;
        }
    }
