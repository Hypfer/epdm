...
{
  public:
    typedef void (*WaitCallback)();  // Added: callback definition
    ...
    GxEPD2_EPD(int8_t cs, int8_t dc, int8_t rst, int8_t busy, int8_t busy_level, uint32_t busy_timeout,
               uint16_t w, uint16_t h, GxEPD2::Panel p, bool c, bool pu, bool fpu, WaitCallback callback = nullptr); // Modified: added callback parameter
  ...
  protected:
    ...
    WaitCallback _waitCallback;  // Added: callback member
};
